...
${interests.map((i) => `          <li>• ${escapeHtml(i)}</li>`).join('\n')}
  ...
  // GitHub is optional: printed right after LinkedIn when the master has one.
  const github = String(p.github || '').trim();
  const githubLink = github ? `<a href="https://${escapeHtml(github)}">${escapeHtml(github)}</a>` : '';
  const githubRow = githubLink
    ? `<div class="h-row">
